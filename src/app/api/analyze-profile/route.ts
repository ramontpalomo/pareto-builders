...
${projects?.map((p: {title:string;description:string;results?:string}) => `- ${p.title}: ${p.description?.slice(0,100)}... Resultados: ${p.results || 'não informados'}`).join('\n') || 'Nenhum projeto cadastrado'}

CERTIFICAÇÕES (${certifications?.length || 0}):
${certifications?.map((c: {name:string;issuer:string;is_fma?:boolean}) => `- ${c.name} (${c.issuer})${c.is_fma ? ' [FMA]' : ''}`).join('\n') || 'Nenhuma certificação'}

Responda APENAS com JSON válido:
{
  "score": <0-100>,
  "nivel": "<Iniciante|Em desenvolvimento|Bom|Ótimo|Excelente>",
  "resumo": "<frase de 1 linha>",
  "sugestoes": [
    {
...
Máximo 6 sugestões priorizadas por impacto.`
    ...
    console.error('Erro no analyze-profile:', error)
